...
public class ExampleGraphView : GraphView
{
    ...
    public override List<Port> GetCompatiblePorts(Port startPort, NodeAdapter nodeAdapter)
    {
        ...
        {
            // compatiblePortに追加する条件（startPortから繋げる条件）

            // 同じノードには繋げない
            if (startPort.node == port.node) return false;
            // Input同士、Output同士は繋げない
            if (startPort.direction == port.direction) return false;
            // ポートの型が一致していない場合は繋げない
            if (startPort.portType != port.portType) return false;
